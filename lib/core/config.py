...
config.SIAMFC.DATASET.COCO = edict()
config.SIAMFC.DATASET.DET = edict()
config.SIAMFC.DATASET.LASOT = edict()
config.SIAMFC.DATASET.YTB = edict()
config.SIAMFC.DATASET.VISDRONE = edict()
...
config.SIAMFC.DATASET.GRAY = 0
config.SIAMFC.DATASET.MIXUP = 0
config.SIAMFC.DATASET.CUTOUT = 0
config.SIAMFC.DATASET.CHANNEL6 = 0
config.SIAMFC.DATASET.LABELSMOOTH = 0
config.SIAMFC.DATASET.ROTATION = 0
config.SIAMFC.DATASET.SHIFTs = 64
config.SIAMFC.DATASET.SCALEs = 0.18
...
# got10k
config.SIAMFC.DATASET.GOT10K.PATH = '/data/share/LARGESIAM/got10k/crop511'
config.SIAMFC.DATASET.GOT10K.ANNOTATION = '/data/share/LARGESIAM/got10k/train.json'
config.SIAMFC.DATASET.GOT10K.RANGE = 100
config.SIAMFC.DATASET.GOT10K.USE = 200000

# visdrone
config.SIAMFC.DATASET.VISDRONE.ANNOTATION = '/data2/SMALLSIAM/visdrone/train.json'
config.SIAMFC.DATASET.VISDRONE.PATH = '/data2/SMALLSIAM/visdrone/crop271'
config.SIAMFC.DATASET.VISDRONE.RANGE = 100
config.SIAMFC.DATASET.VISDRONE.USE = 100000

# train
config.SIAMFC.TRAIN.GROUP = "resrchvc"
config.SIAMFC.TRAIN.EXID = "vot19_ex5_setting_default"
config.SIAMFC.TRAIN.MODEL = "SiamFCRes22W"
...
config.SIAMFC.TUNE.METHOD = 'TPE'  # 'GENE' or 'RAY'

# TODO(zhangzhipeng2017@ia.ac.cn): add siamrpn train config
# #-----————- config for siamrpn ------------
...
# train
config.SIAMRPN.TRAIN.GROUP = "resrchvc"
config.SIAMRPN.TRAIN.EXID = "vot19_ex5_setting_default"
...
config.SIAMRPN.TRAIN.PRETRAIN = 'resnet23_inlayer.model'
config.SIAMRPN.TRAIN.LR_POLICY = 'log'
config.SIAMRPN.TRAIN.LR = 0.01
config.SIAMRPN.TRAIN.LR_END = 0.000001
config.SIAMRPN.TRAIN.MOMENTUM = 0.9
config.SIAMRPN.TRAIN.WEIGHT_DECAY = 0.0001
config.SIAMRPN.TRAIN.CLS_WEIGHT = 1
config.SIAMRPN.TRAIN.REG_WEIGHT = 1
config.SIAMRPN.TRAIN.WHICH_USE = ['VID', 'YTB']  # VID or 'GOT10K'
config.SIAMRPN.TRAIN.ANCHORS_RATIOS = [0.33, 0.5, 1, 2, 3]  # VID or 'GOT10K'
config.SIAMRPN.TRAIN.ANCHORS_SCALES = [8]  # VID or 'GOT10K'
...
config.SIAMRPN.TUNE.METHOD = 'TPE'  # 'GENE' or 'TPE'
